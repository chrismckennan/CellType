...
D.m <- t(chol(Lambda.m))
L.m.stand <- L.m %*% D.m
Omega.avh <- C.all[1:5,] %*% cov.mat.cell[,1:2] %*% solve(t(cov.mat.cell[,1:2]) %*% cov.mat.cell[,1:2])
...
X.tmp <- (diag(n) - t(C.all[1:5,]) %*% solve(C.all[1:5,] %*% t(C.all[1:5,]), C.all[1:5,])) %*% cov.mat.cell[,1:2]
...
```{r PlotDifferences}
h.nocell <- hist(p.values.nocell.group, plot=T, main="Differences in P-values With and Without Cell Type", xlab="P-values for Amish vs. Hutterite", ylab="Number of Sites")
h.cell <- hist(p.values.cell.group, plot=F)

for (i in 1:length(h.cell$mids)) {
  lines(rep(h.cell$mids[i], 100), seq(0,h.cell$counts[i],length=100), col="red", lwd=2)
}
legend("topright", legend=c("With Cell", "Without Cell"), fill=c("red", "black"))
```
...
Gamma.und <- matrix(rnorm(r.und*p.und), nrow=p.und, ncol=r.und) * sqrt(1/n.und) * sqrt(0.15)
...
Below I will run simulations for various sizes of $\Gamma$ to see when we start underestimating $\Omega$. These simulations assume that everything is nomally distributed.
```{r UnderestimateOmega_all_Normal}
p.und <- floor(327273 / 10)
r.und <- 3
n.und <- 100
n.repeat <- 50
...
##Plot results##
jpeg(filename = "UnderestimatingOmega_Multipliers.jpeg", width=1056, height=629)
par(mfrow=c(3,3), mex=0.85, cex=0.8)
for (i in 1:(length(mult.vec)-1)) {
  x.i <- norm.true.mat[,i]
  y.i <- norm.est.mat[,i]
...
Here we test to see if we underestimate $\Omega$ when we can estimate some of the confounders.
```{r UnderestimateOmega_all_Normal_Theorem}
p.und <- floor(327273 / 10)
r.und <- 3
n.und <- 200
n.repeat <- 50
...
Sigma.und <- 1/rgamma(p.und, shape=6, rate=6)
Sigma.und <- rep(1, p.und)
for (i in 1:ncol(mult.mat)) {
  mult.vec.i <- mult.mat[,i]
  Gamma.und <- sweep(matrix(rnorm(r.und*p.und), nrow=p.und, ncol=r.und) * sqrt(Sigma.und), MARGIN=2, sqrt(mult.vec.i/n.und), '*')
  V.und <- svd(t(Gamma.und / Sigma.und) %*% Gamma.und)$v
  Gamma.und <- Gamma.und %*% V.und
  for (j in 1:n.repeat) {
    X.und <- cbind(rep(1,n.und), rbinom(n=n.und, size=1, prob=0.5))
    alpha.und <- matrix(sqrt(n.und)*c(0.9, 0.9, 0.9), nrow=r.und, ncol=1)
    out.est <- CompareEstimate(X.und, alpha.und, Gamma.und, Sigma.und)
    norm.true.mat[j,i] <- t(out.est$alpha.true) %*% out.est$alpha.true
    norm.est.mat[j,i] <- t(out.est$alpha.hat) %*% out.est$alpha.hat
...
dev.off()
```
My prediction appears to be correct. Not only that, but $\left( \hat{\Gamma}^T\hat{\Sigma}^{-1}\hat{\Gamma} \right)^{-1}\hat{\Gamma}^T \hat{\Sigma}^{-1}\Gamma \to \text{diag}\left( \frac{\lambda_1}{\lambda_1 + 1}, \ldots, \frac{\lambda_r}{\lambda_r + 1} \right)$ when $n, p, \frac{p}{n} \to \infty$.

The below simulates data according to a T-distribution. Below, I will assume that the confounders are normally distributed and the residuals are T-distribution with 5 degrees of freedom. This is so the residuals have finite excess kurtosis.
```{r UnderestimateOmega_all_ET4p1}
df.t.residual <- 3.0
p.und <- floor(327273 / 10)
r.und <- 3
n.und <- 1000
...
rm(E.ij)
##Plot results##
jpeg(filename = "UnderestimatingOmega_Multipliers_ET5.jpeg", width=1056, height=629)
par(mfrow=c(3,3), mex=0.85, cex=0.8)
for (i in 1:(length(mult.vec)-1)) {
  x.i <- norm.true.mat[,i]
  y.i <- norm.est.mat[,i]
...
The below simulates data according to a T-distribution. Below, I will assume that the confounders are normally distributed and the residuals are T-distribution with 2.1 degrees of freedom. This is so the residuals have infinite excess kurtosis.
```{r UnderestimateOmega_all_ET2p1}
df.t.residual <- 2.1
p.und <- floor(327273 / 10)
r.und <- 3
n.und <- 100
...
rm(E.ij)
##Plot results##
jpeg(filename = "UnderestimatingOmega_Multipliers_ET2p1.jpeg", width=1056, height=629)
par(mfrow=c(3,3), mex=0.85, cex=0.8)
for (i in 1:(length(mult.vec)-1)) {
  x.i <- norm.true.mat[,i]
  y.i <- norm.est.mat[,i]
...
This section assume that the multiplier for the information matrix is 1.3 and the correlation is 1.41
```{r FDRplot_neutro1}
p.neutro <- floor(nrow(M.orig) / 10)
n.neutro <- 100
mult.neutro <- 1.3
pi0.L.neutro <- 0.7
corr.neutro <- Omega.avh[4,2] / sqrt(Lambda.m[4,4])
pi0.B.neutro <- 0.95; stand.B.neutro <- 0.4
n.rep <- 50

#Sigma and L#

Sigma.neutro <- rep(0.15, p.neutro)
L.neutro <- matrix(rbinom(p.neutro*1, size=1, prob=1-pi0.L.neutro), nrow=p.neutro, ncol=1) * matrix(rnorm(p.neutro*1), nrow=p.neutro, ncol=1) * sqrt(Sigma.neutro) * sqrt(mult.neutro / n.neutro) * sqrt(1 / (1-pi0.L.neutro))

#B, X#

B.neutro <- cbind(rep(0,p.neutro), rbinom(p.neutro*1, size=1, prob=1-pi0.B.neutro) * rnorm(p.neutro*1) * sqrt(0.15 * stand.B.neutro))
X.neutro <- rbind(rep(1, n.neutro), c(rep(1, n.neutro/2), rep(0, n.neutro/2)))
orthog.neutro <- diag(n.neutro) - t(X.neutro) %*% solve(X.neutro %*% t(X.neutro), X.neutro)

#Simulate and analyze data#

q.values.int <- seq(0.05, 0.30, by=0.05)
fdp.neutro.cell <- matrix(NA, nrow=n.rep, ncol=length(q.values.int))
fdp.neutro.nocell <- matrix(NA, nrow=n.rep, ncol=length(q.values.int))
alpha.under.neutro <- rep(NA, n.rep)
for (i in 1:n.rep) {
  C.neutro <- corr.neutro * rbind(X.neutro[2,]) + matrix(rnorm(n.neutro*1), nrow=1, ncol=n.neutro) %*% orthog.neutro
  Sim.neutro <- Sim.Analyze.Fdr(X.neutro, C.neutro, B.neutro, L.neutro, Sigma.neutro)
  alpha.under.neutro[i] <- t(Sim.neutro$alpha.cate) %*% t(Sim.neutro$alpha.cate) / corr.neutro^2
  for (j in 1:length(q.values.int)) {
    fdp.neutro.cell[i,j] <- Sim.neutro$fdp.cell[which.min(abs(sort(Sim.neutro$q.cell$qvalues) - q.values.int[j]))]
    fdp.neutro.nocell[i,j] <- Sim.neutro$fdp.nocell[which.min(abs(sort(Sim.neutro$q.nocell$qvalues) - q.values.int[j]))]
  }
}

#Get summary statistics of results#
FDR.neutro.cell <- apply(fdp.neutro.cell, 2, mean)
sd.fdp.neutro.cell <- apply(fdp.neutro.cell, 2, sd)

FDR.neutro.nocell <- apply(fdp.neutro.nocell, 2, mean)
sd.fdp.neutro.nocell <- apply(fdp.neutro.nocell, 2, sd)

mean.und.neutro <- mean(alpha.under.neutro)
sd.und.neutro <- sd(alpha.under.neutro)

###Plot summary level data###
n.stand.plot <- 2
text.diff <- 0.01
line.wid.sd <- 0.005
text.size <- 0.6
...
#With cell (blue)#
plot(rep(q.values.int[1], 50), seq(FDR.neutro.cell[1] - n.stand.plot*sd.fdp.neutro.cell[1], FDR.neutro.cell[1] + n.stand.plot*sd.fdp.neutro.cell[1], length=50), type="l", col="blue", xlab="q-value cutoff", ylab="False Discovery Proportion (FDP)", main="FDP Plot for Simulation (a)", xlim=c(0,max(q.values.int)), ylim=c(0, 0.55))
lines(seq(q.values.int[1] - line.wid.sd, q.values.int[1] + line.wid.sd, length=50), rep(FDR.neutro.cell[1] - n.stand.plot*sd.fdp.neutro.cell[1], 50), col="blue")
lines(seq(q.values.int[1] - line.wid.sd, q.values.int[1] + line.wid.sd, length=50), rep(FDR.neutro.cell[1] + n.stand.plot*sd.fdp.neutro.cell[1], 50), col="blue")
points(q.values.int[1], FDR.neutro.cell[1], pch="x", col="blue")

#Without cell (red)#
lines(rep(q.values.int[1], 50), seq(FDR.neutro.nocell[1] - n.stand.plot*sd.fdp.neutro.nocell[1], FDR.neutro.nocell[1] + n.stand.plot*sd.fdp.neutro.nocell[1], length=50), col="red")
lines(seq(q.values.int[1] - line.wid.sd, q.values.int[1] + line.wid.sd, length=50), rep(FDR.neutro.nocell[1] - n.stand.plot*sd.fdp.neutro.nocell[1], 50), col="red")
lines(seq(q.values.int[1] - line.wid.sd, q.values.int[1] + line.wid.sd, length=50), rep(FDR.neutro.nocell[1] + n.stand.plot*sd.fdp.neutro.nocell[1], 50), col="red")
points(q.values.int[1], FDR.neutro.nocell[1], pch="x", col="red")
text(x=q.values.int[1] - text.diff, y=FDR.neutro.nocell[1], paste0("(", as.character(round(q.values.int[1], digits=2)), ", ", as.character(round(FDR.neutro.nocell[1], digits=2)), ")"), col="red", cex=text.size)

#Plot the rest#
for (j in 2:length(q.values.int)) {
  #Without cell (red)#
	lines(rep(q.values.int[j], 50), seq(FDR.neutro.nocell[j] - n.stand.plot*sd.fdp.neutro.nocell[j], FDR.neutro.nocell[j] + n.stand.plot*sd.fdp.neutro.nocell[j], length=50), col="red")
	lines(seq(q.values.int[j] - line.wid.sd, q.values.int[j] + line.wid.sd, length=50), rep(FDR.neutro.nocell[j] - n.stand.plot*sd.fdp.neutro.nocell[j], 50), col="red")
	lines(seq(q.values.int[j] - line.wid.sd, q.values.int[j] + line.wid.sd, length=50), rep(FDR.neutro.nocell[j] + n.stand.plot*sd.fdp.neutro.nocell[j], 50), col="red")
	points(q.values.int[j], FDR.neutro.nocell[j], pch="x", col="red")
	text(x=q.values.int[j] - text.diff, y=FDR.neutro.nocell[j], paste0("(", as.character(round(q.values.int[j], digits=2)), ", ", as.character(round(FDR.neutro.nocell[j], digits=2)), ")"), col="red", cex=text.size)
	
	#With cell (blue)#
	lines(rep(q.values.int[j], 50), seq(FDR.neutro.cell[j] - n.stand.plot*sd.fdp.neutro.cell[j], FDR.neutro.cell[j] + n.stand.plot*sd.fdp.neutro.cell[j], length=50), col="blue")
	lines(seq(q.values.int[j] - line.wid.sd, q.values.int[j] + line.wid.sd, length=50), rep(FDR.neutro.cell[j] - n.stand.plot*sd.fdp.neutro.cell[j], 50), col="blue")
	lines(seq(q.values.int[j] - line.wid.sd, q.values.int[j] + line.wid.sd, length=50), rep(FDR.neutro.cell[j] + n.stand.plot*sd.fdp.neutro.cell[j], 50), col="blue")
	points(q.values.int[j], FDR.neutro.cell[j], pch="x", col="blue")
}

abline(a=0, b=1)
legend("bottomright", legend=c("Without cell type", "With cell type"), fill=c("red", "blue"))
```
