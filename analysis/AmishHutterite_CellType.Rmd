...
# hist(exp(M[,1])/(1 + exp(M[,1])), xlab="m/(m + u)", ylab="Density of CpG Sites", main="Histogram of Methylation", prob=T)
```
...
Omega.sim <- Omega.sim[1:K.sim, 1:(d.sim+1)]
alpha.cell.sim <- (1/var.cell - 1)
sigma.L.sim <- 2*c(1, 1, 1) / Omega.QL[1:3,1] * sqrt(0.15)
...
analyzed.data <- AnalyzeData(M.sim, data.sim, r.sim, r.sim+1, B.sim, L.sim)
plot(sort(analyzed.data$q.cell$qvalue), analyzed.data$fsr.cell$fdr, xlab="Est. Q-value", ylab="True FDR", main="FDR Plot With and Without Cell Type", type="l", ylim=c(0,0.3), xlim=c(0,0.3))
lines(sort(analyzed.data$q.nocell$qvalue), analyzed.data$fsr.nocell$fdr, col="red")
#legend("bottomright", legend=c("With Cell", "Without Cell"), fill=c("black", "red"))
abline(a=0,b=1, col="violet")
```
...
The asymptotic theory for $L$ when cell type is unobserved is given in terms of $\ell \Vert R \Vert_2$. When $X$ and cell type are highly correlated, $\Vert R \Vert_2$ is small, meaning $\ell \Vert R \Vert_2$ is small. Therefore, the coefficient of variation for $\ell \Vert R \Vert_2$ is small and we would estimate $\frac{1}{\Vert R \Vert_2} \omega$ to be far smaller than it actually is (regression towards the mean). I confirmed this by performing OLS with $\tilde{Y} \sim \hat{\Gamma}$ with cate output:

```{r RegressionTowardMean}
Q.tmp <- qr.Q(qr(X.sim), complete=T)
R.tmp <- qr.R(qr(X.sim))
Y.tilde.sim <- M.sim %*% Q.tmp
alpha.ols <- 1/sum(analyzed.data$cate.nocell$Gamma * analyzed.data$cate.nocell$Gamma) * sum(analyzed.data$cate.nocell$Gamma * (Y.tilde.sim[,1:3] %*% solve(t(R.tmp)))[,3])
plot(Mean.conf.true[,3], alpha.ols * analyzed.data$cate.nocell$Gamma, xlab="Mean of the True Confounder", ylab="Mean of the Confounder WITHOUT Cell Type Est. with OLS"); abline(a=0,b=1,col="red")
```

## Penalized Robust Regression
Below I will attempt to correct the underestimation in $\alpha$ by using a penalized robust regression. The loss function I will minimize is
\[
l(\alpha) = \sum\limits_{j=1}^p \rho\left( \frac{\tilde{Y}_j - \hat{\Gamma}_j^T \alpha}{\hat{\sigma}_j} \right) + \frac{q}{2} \left( \alpha - \hat{\Omega} \right)^T V^{-1}\left( \alpha - \hat{\Omega} \right)
\]
where $\hat{\Omega}$ was estimated via quasi-likelihood and $V = \mathcal{I}\left( \hat{\Omega} \right)^{-1}$. For $q = 1$, this is akin to finding the maximum aposteriori estimate for $\alpha$ (Bayesian central limit theorem).


The below R code is a proof of principle of the above concept. I just want to see if we can get a better estimate of $\alpha$ by using cell type training data at this step.
```{r PenHuber_POP}
Gamma.hub <- analyzed.data$cate.nocell$Gamma
Omega.hub <- t(solve(t(X.sim) %*% X.sim) %*% t(X.sim) %*% C.sim)
V.x.hub <- solve(t(X.sim) %*% X.sim)    #Int, Cov1, Cov2 (I am interested in Cov2)
V.obs.hub <- 1/(n.sim - d.sim - 1) * t(C.sim - X.sim %*% t(Omega.sim)) %*% (C.sim - X.sim %*% t(Omega.sim))
inv.V.total <- kronecker(t(X.sim) %*% X.sim, solve(V.obs.hub))     #(d+1)K x (d+1)K matrix. It runs in blocks of size K x K
inv.V.use <- inv.V.total[9,9]    #r = 1 for now

Y.tilde.hub <- M.sim %*% Q.tmp
Y.tilde.hub.use <- Y.tilde.hub[,1:(d.sim+1)] %*% solve(t(R.tmp))    #I want Cov2, which is column 3
...
##It looks like we can do far better by choosing only the most informative sites for cell types##
...
##However, those sites are difficult to choose. If our estimate for L is noisy, then q-values are not an appropriate way to choose informative sites.
```
If we use only the sites with large non-zero values for $L$ in the robust regression, we do a good job at recovering the true correlation. However, when the estimate for $L$ is noisy (as it will be when robust regression fails), we cannot identify the informative site and this technique fails. Maybe we can use this idea to use sites we know that change to 


## When Does RR Fail?
Here I will study when robust regression fails. Recall that the confounding is
\[
\ell \Vert R \Vert_2 \left[ \frac{1}{\Vert R \Vert_2} \omega^T X + N\left( 0,I_n \right) \right].
\]

Below I will get estimates of the above parameters with the original neutrophil simulation
```{r NeutroParams}
...
```
Note that the expected size of $L$ and $\frac{\sigma}{\sqrt{n}}$ are roughly the same size. 

## Apply Houseman

```{r Houseman}
B.naive <- M.sim %*% X.sim %*% solve(t(X.sim) %*% X.sim)
E.naive <- M.sim - B.naive %*% t(X.sim)
svd.naive <- svd(E.naive)
u.house <- svd.naive$u[,1+r.sim]
B.house <- B.naive - cbind(u.house) %*% (rbind(u.house) %*% B.naive)
plot(B.sim[,2], B.house[,3]); abline(a=0,b=1,col="red")
```
Houseman tends to give slightly inflated values for $B$. This expected, since $L$ is difficult to estimate.

## Proof of Principal for the Plugin Method

```{r PluginPOP}
f.cell <- 0.5       #Fraction of individuals whose cell type I observe
n.1 <- floor(n.sim * f.cell)
n.2 <- n.sim - n.1
ind.1 <- sample(1:n.sim, n.1, replace=F)

X.sim.1 <- X.sim[ind.1,]     #n.1 x d (includes intercept)
X.sim.2 <- X.sim[-ind.1,]    #n.2 x d
C.sim.1 <- C.sim[ind.1,]     #n.1 x K
Y1.sim <- M.sim[,ind.1]      #p x n.1
Y2.sim <- M.sim[,-ind.1]     #p x n.2
qr.X1 <- qr(X.sim.1)
qr.X2 <- qr(X.sim.2)
Q.X1 <- qr.Q(qr.X1, complete=T)    #n.1 x n.1
Q.X2 <- qr.Q(qr.X2, complete=T)    #n.2 x n.2
R.X1 <- qr.R(qr.X1)
R.X2 <- qr.R(qr.X2)

Y1.sim.tilde <- Y1.sim %*% Q.X1
Y2.sim.tilde <- Y2.sim %*% Q.X2
F.mat.total <- t(C.sim.1) %*% Q.X1    #K x n.1
F.mat.1 <- F.mat.total[,1:ncol(X.sim.1)]
F.mat.2 <- F.mat.total[,(ncol(X.sim.1)+1):n.1]    #K x (n.1 - d)
Z1.sim <- Y1.sim.tilde[,(ncol(X.sim.1)+1):n.1]
Z2.sim <- Y2.sim.tilde[,(ncol(X.sim.2)+1):n.2]
F.mat.sim <- F.mat.2   #Note there is no need to center these matrices (see likelihood in LaTeX)

m1.sim <- n.1 - ncol(X.sim.1)
m2.sim <- n.2 - ncol(X.sim.2)
L.sim.start <- Y1.sim.tilde[,(ncol(X.sim.1)+1):n.1] %*% t(F.mat.2) %*% solve(F.mat.2 %*% t(F.mat.2))
Lambda.start <- F.mat.sim %*% t(F.mat.sim) / m1.sim
Sigma.start <- Sigma.sim

EM.POP <- PEM(Z1=Z1.sim, Z2=Z2.sim, F.mat=F.mat.sim, L.0=L.sim.start, Lambda.0=Lambda.start, Gamma.0=NULL, Sigma.0=Sigma.start, m1=m1.sim, m2=m2.sim, tol = 3e-6)
if (is.null(EM.POP$blup.V)) {
  tmp.matrices <- Compute.Matrices(EM.POP$L, EM.POP$Lambda, EM.POP$Gamma, EM.POP$Sigma, Z1.sim, Z2.sim, F.mat.sim)
  V.blup.sim <- EM.POP$Lambda %*% tmp.matrices$G1
} else {
  V.blup.sim <- EM.POP$blup.V
}
C.2.blup <- V.blup.sim %*% t(Q.X2[,(ncol(X.sim.2)+1):n.2]) + t(C.sim.1) %*% X.sim.1 %*% solve(t(X.sim.1) %*% X.sim.1) %*% t(X.sim.2)
mat.cov.pop <- cbind( rbind(X.sim.1, X.sim.2), rbind(C.sim.1, t(C.2.blup)) )
var.pop <- solve(t(mat.cov.pop) %*% mat.cov.pop)
Sigma.pop <- EM.POP$Sigma

beta.pop <- cbind(Y1.sim, Y2.sim) %*% mat.cov.pop %*% solve(t(mat.cov.pop) %*% mat.cov.pop)
dof.pop <- n.sim - ncol(mat.cov.pop)
pvalues.pop <- 2 - 2 * pt( abs(beta.pop[,3]) / sqrt(Sigma.pop) / sqrt(var.pop[3,3]), df=dof.pop )
qvalue.pop <- qvalue(pvalues.pop)
fsr.pop <- false.sign.results(B.sim[,2], beta.pop[,3], qvalue.pop$qvalue)
lines(sort(qvalue.pop$qvalue), fsr.pop$fdr, col="blue")
legend("bottomright", legend=c("With Cell", "Without Cell", "50% With Cell"), fill=c("black", "red", "blue"))
```
Using 50% of the cell type data works when the confounding is normal or Dirichlet, provided I know the parameter $\Gamma$. Estimating $\Gamma$ is difficult in practice.


## Test Performance of EM Algorithm

Here I will test the performance of my EM algorithm using my proposed starting point for $\Gamma$ (see LaTeX document).

```{r SimSynthData_SparseLSparseB_Ind_Neutrophils_Dirichlet_EMtest, echo=T, eval=T}
set.seed(1988)
n.sim <- 200   #Number of individuals
r.sim <- 4     #Number of additional confounders
K.sim <- 3     #Number of cell types
...
Omega.sim <- Omega.sim[1:K.sim, 1:(d.sim+1)]
alpha.cell.sim <- (1/var.cell - 1)
sigma.L.sim <- 1.5*c(1, 1, 1) / Omega.QL[1:3,1] * sqrt(0.15)
...
Use the above simulation as input into the EM algorithm
```{r TestEMStartingPoints}
f.cell <- 0.7       #Fraction of individuals whose cell type I observe
n.1 <- floor(n.sim * f.cell)
n.2 <- n.sim - n.1
ind.1 <- sample(1:n.sim, n.1, replace=F)

###write simulations to disk###
path.dir <- "/Users/Chris/Desktop/Uchicago/Nicolae/GitWork/CellType/data/Simulations_160608"

#Y#
path.Y.out <- file.path(path.dir, "Msim.txt")
write.table(M.sim, path.Y.out, sep="\t", col.names=F, row.names=F, append=F, quote=F)

#X and C#
path.Cov.out <- file.path(path.dir, "Covsim.txt")
write.table(data.sim, path.Cov.out, sep="\t", col.names=T, row.names=F, append=F, quote=F)

#Omega#
path.Omega.out <- file.path(path.dir, "Omegasim.txt")
write.table(Omega.sim, path.Omega.out, sep="\t", col.names=T, row.names=T, append=F, quote=F)

#L#
path.L.out <- file.path(path.dir, "Lsim.txt")
write.table(L.sim, path.L.out, sep="\t", col.names=F, row.names=F, append=F, quote=F)

#B#
path.B.out <- file.path(path.dir, "Bsim.txt")
write.table(B.sim, path.B.out, sep="\t", col.names=F, row.names=F, append=F, quote=F)

#Gamma#
path.Gamma.out <- file.path(path.dir, "Gammasim.txt")
write.table(Gamma.sim, path.Gamma.out, sep="\t", col.names=F, row.names=F, append=F, quote=F)

#Sigma#
path.Sigma.out <- file.path(path.dir, "Sigmasim.txt")
write.table(rbind(Sigma.sim), path.Sigma.out, sep="\t", col.names=F, row.names=F, append=F, quote=F)

#ind.1#
path.ind1.out <- file.path(path.dir, "ind1sim.txt")
write.table(rbind(ind.1), path.ind1.out, sep="\t", col.names=F, row.names=F, append=F, quote=F)
...
Q.X1 <- qr.Q(qr.X1, complete=T)    #n.1 x n.1
Q.X2 <- qr.Q(qr.X2, complete=T)    #n.2 x n.2
Q.X.total <- qr.Q(qr(X.sim), complete=T)    #n.sim x n.sim
R.X1 <- qr.R(qr.X1)
R.X2 <- qr.R(qr.X2)
...
fa.em.sim <- fa.em( t( M.sim %*% Q.X.total[,(ncol(X.sim)+1):n.sim] ), r=r.sim+1 )
Sigma.use <- fa.em.sim$Sigma
theta.start <- Starting.Points(Z1.sim, Z2.sim, F.mat.sim, Sigma.use, r.cell=r.sim)
Gamma.start <- theta.start$Gamma.0.2
L.start <- theta.start$L.0
Lambda.start <- theta.start$Lambda.0

EM.nosigma <- PEM(Z1.sim, Z2.sim, F.mat.sim, L=L.start, Lambda=Lambda.start, Gamma=Gamma.start, Sigma=Sigma.use, m1.sim, m2.sim, update.Gamma=F, update.Sigma=F, update.Lambda=F, tol=1e-6)
rm(L.start); rm(Gamma.start); rm(Lambda.start); rm(theta.start)
L <- EM.nosigma$L
Gamma <- EM.nosigma$Gamma
Lambda <- EM.nosigma$Lambda


#EM.sigma <- PEM(Z1.sim, Z2.sim, F.mat.sim, L=L.start, Lambda=Lambda.start, Gamma=Gamma.start, Sigma=Sigma.use, m1.sim, m2.sim, update.Gamma=T, update.Sigma=T, update.Lambda=F, tol=5e-6)

V.blup.sim <- EM.nosigma$blup.V
W1.2.blup.sim <- EM.nosigma$blup.W1
W2.2.blup.sim <- EM.nosigma$blup.W2

###Estimate the correlation with Gamma###

#Solve for alpha.1#
QR.C.sim.1 <- qr(C.sim.1)
Q.C.sim.1 <- qr.Q(QR.C.sim.1, complete=T)
Q.C.sim.1.kern <- Q.C.sim.1[,(ncol(C.sim.1)+1):n.1]
Y1.mc <- Y1.sim %*% Q.C.sim.1.kern
X1.mc <- t(X.sim.1) %*% Q.C.sim.1.kern     #d x (n.1 - K) matrix
Q.X1mc <- qr.Q(qr(t(X1.mc)), complete=T)
R.X1mc <- qr.R(qr(t(X1.mc)))

Y1Rmt.mc <- Y1.mc %*% Q.X1mc[,1:ncol(X.sim.1)] %*% solve(t(R.X1mc))
...
#Solve for alpha.2#
Y2Rmt <- Y2.sim %*% X.sim.2 %*% solve(t(X.sim.2) %*% X.sim.2) - cbind(L[,3]) %*% rbind((t(C.sim.1) %*% X.sim.1 %*% solve(t(X.sim.1) %*% X.sim.1))[3,])
...
W1.blup <- EM.nosigma$blup.W1 %*% t(Q.X1[,(ncol(X.sim.1)+1):n.1]) + alpha.1 %*% t(X.sim.1)
W2.blup <- EM.nosigma$blup.W2 %*% t(Q.X2[,(ncol(X.sim.2)+1):n.2]) + alpha.2 %*% t(X.sim.2)
...
beta.pop <- cbind(Y1.sim, Y2.sim) %*% mat.cov.pop %*% solve(t(mat.cov.pop) %*% mat.cov.pop)
orthog.pop <- diag(n.sim) - mat.cov.pop %*% solve(t(mat.cov.pop) %*% mat.cov.pop) %*% t(mat.cov.pop)
Sigma.pop <- rowSums((cbind(Y1.sim, Y2.sim) %*% orthog.pop) * cbind(Y1.sim, Y2.sim)) / dof.pop
pvalues.pop <- 2 - 2 * pt( abs(beta.pop[,3]) / sqrt(Sigma.pop) / sqrt(var.pop[3,3]), df=dof.pop )
qvalue.pop <- qvalue(pvalues.pop)
fsr.pop <- false.sign.results(B.sim[,2], beta.pop[,3], qvalue.pop$qvalue)
lines(sort(qvalue.pop$qvalue), fsr.pop$fdr, col="blue")
...
```
The above uses an oracle estimate for $\Gamma$ and estimates $L$ using EM. We see that we can still control the FDR at a nominal level with our procedure with little loss in power. We do far better than using only the 70% of the individuals with observed cell type.

I will now do the exact same as above, but will estimate $\Gamma$ from the data. It looks like controlling for the estimated BLUPs gives us a much better starting point than only using the fraction with observed cell type. We do marginally better in our starting point for $\Gamma$ when we project the cell types onto the top CCA component. The $R^2$ for the regressions onto $\text{diag}\left( \Gamma \Gamma^T \right)$ were 0.8546 for 70% cell, 0.8963 for 70% 3 cells + BLUP, 0.8993 for 70% projected cells + BLUP, 0.9039 for 100% cell. Note that we are overestimating the variance due to confounding by about 6.8% and underestimating the additional biological variance by about 2.3%. This may be more significant with additional confounders, which is what David observed in his simulations.
