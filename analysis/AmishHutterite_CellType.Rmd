...
```{r InitSimData, include=F, eval=F}
K.all <- nrow(C.all) - 1
...
```{r InvestigateCellEffect, eval=F}
...
```{r SimSynthData_SparseLSparseB_Ind}
set.seed(1988)
...
```{r SimSynthData_SparseLSparseB_Ind_LargeL0, echo=F}
set.seed(1988)
n.sim <- 200   #Number of individuals
r.sim <- 5     #Number of additional confounders
K.sim <- 3     #Number of cell types
...
```{r SimSynthData_SparseLSparseB_Ind_LargeL1, echo=F}
set.seed(1988)
...
```{r SimSynthData_SparseLSparseB_Ind_LargeL5, echo=F}
set.seed(1988)
n.sim <- 400   #Number of individuals
r.sim <- 5     #Number of additional confounders
K.sim <- 3     #Number of cell types
...
```{r SimSynthData_SparseLSparseB_Ind_LargeL9, echo=F, eval=T}
set.seed(1988)
...
```{r SimSynthData_SparseLSparseB_Ind_LargeL10, echo=F, eval=T}
set.seed(1988)
n.sim <- 100   #Number of individuals
r.sim <- 5     #Number of additional confounders
K.sim <- 3     #Number of cell types
...
Here I assume that $L$ and $B$ are both sparse. The placenet of the 0's is independent in both $L$ and $B$. The standardized effect size for $L\Omega X$ is $(0.75, 0.75, 0.75)$.
```{r SimSynthData_SparseLSparseB_Ind_LargeL11, echo=F, eval=T}
set.seed(1988)
n.sim <- 100   #Number of individuals
r.sim <- 5     #Number of additional confounders
K.sim <- 3     #Number of cell types
...
```{r SimSynthData_SparseLSparseB_Ind_LargeL12, echo=F, eval=T}
set.seed(1988)
n.sim <- 100   #Number of individuals
r.sim <- 5     #Number of additional confounders
K.sim <- 3     #Number of cell types
...
3.) If I reduce the residual variance of my residuals while keeping the confounders constant, CATE does VERY poorly. If I reduct both to almost 0, I see the same effect had I not changed anything. I can therefore conclude that this is not a problem with sample size.
4.) If I keep the Eos standardized effect size to 0.19, drastically reduce the residual variance, and decrease the standardize effect size of $B$ to 1/2 that of $L \Omega$, 
...
7.) When more than one effect is non-zero but sparse ($\pi_0 = 0.7$), I see an even LARGER effect.
