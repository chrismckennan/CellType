...
beta.values <- getBeta(Mset.swan.norm)
```
...
Fix $\Omega$, $X_1$, $X_2$, $C_1$, $B$, $L$, $\Sigma$. To make things simple I assume there is only a single covariate of interest (a binary 1 or 0) and 4 cell types.

```{r Initialize}
n.1 <- 600     #Number of individuals with known cell type
n.2 <- 4000    #Number of individuals with unmeasured cell type
p <- 100      #Number of methylation sites
...
Omega.sim = Omega[1:2,1:3]; Omega.sim[2,1] = 0.09
XO.1 <- t(X.1) %*% Omega.sim
XO.2 <- t(X.2) %*% Omega.sim
K <- ncol(Omega.sim)
C.1 <-  array(NA, dim=c(n.1, K))    #A n.1 x K matrix of individual 1's cell types (we condition on this)
for (i in 1:n.1) {
...
V.C.1t <- tmp.Q[, (K+1):n.1]    #C.1' V.C.1t = 0, V.C.1t is n.1 x n.1-K and orthonormal

Cov.1 <- cbind(t(X.1), C.1)
H.1 <- Cov.1 %*% solve( t(Cov.1) %*% Cov.1, t(Cov.1) )
perp.1 <- diag(n.1) - H.1

###Simulate L and B###

var.l <- 1/C.1[n.1,]^2      #L_gi ~ N(0, var.l(i))
var.beta <- rep(1, d)       #B_gi ~ N(0, var.beta(i))
L <- array(NA, dim=c(p,K))
B <- array(NA, dim=c(p,d))
prior.B <- 0.5     #B.asthma ~ prior.B*delta_0 + (1-prior.B)*N(0,var.beta[j])
for (k in 1:K) {
  L[,k] <- sqrt(var.l[k]) * rnorm(p)
}
B[,1] <- sqrt(var.beta[1]) * rnorm(p)
for (j in 2:d) {
  B[,j] <- rbinom(p, 1, 1 - prior.B) * sqrt(var.beta[j]) * rnorm(p)
}
```
...
```{r EstimateOmega}
ML.norm <- MaxLike.NewtonLS(t(X.1), C.1, Omega.sim, var.cell, 1e-8, vfixed=F)
...
Cov.predict <- rbind( cbind( t(X.1), C.1 ), cbind( t(X.2), XO.norm.2 ) )
Cov.predict.2 <- cbind( t(X.2), XO.norm.2 )
perp.2 <- diag(n.2) - Cov.predict.2 %*% solve(t(Cov.predict.2) %*% Cov.predict.2, t(Cov.predict.2))
```

Create variance matrix $V \in \mathbb{R}^{Kn_2 \times Kn_2}$ where $G = I_{n_2} \otimes \ell_g^T V I_{n_2} \otimes \ell_g + \sigma^2 I_{n_2}$.

```{r CreateV_tmp}
V <- array(NA, dim=c(n.2*K, n.2*K))
for (i in 1:n.2) {
  x.i <- cbind(X.2[,i])
  p.i <- t(Omega.norm) %*% x.i

  V.i <- sigma.cell.norm^2 * ( diag(as.vector(p.i)) - p.i %*% t(p.i) )  #ith block is V.i + S.i
  V[((i-1)*K+1):(i*K),((i-1)*K+1):(i*K)] <- V.i
...
for (i in 1:n.iter) {
  ...
  Y.1 <- B %*% X.1 + L %*% t(C.1) + sqrt(var.g) * matrix(rnorm(n.1*p), nrow=p, ncol=n.1)
  
  C.2 <- array(NA, dim=c(n.2, K))   #A n.2 x K matrix of cell type proportions
  for (j in 1:n.2) {
    alpha.j <- 1/var.cell * c(XO.2[j,], 1-sum(XO.2[j,]))
    C.2[j,] <- rdirichlet(1, alpha.j)[1:K]
  }
  Y.2 <- B %*% X.2 + L %*% t(C.2) + sqrt(var.g) * matrix(rnorm(n.2*p), nrow=p, ncol=n.2)
  ...
  for (g in 1:p) {
    ...
    #Method 1#
    l.g.hat <- solve( t(C.1) %*% orthog.X.1 %*% C.1, t(C.1) ) %*% orthog.X.1 %*% y.1.g
    beta.list <- Estimator.1(var.hat.g, C.1, V.C.1t, orthog.X.1, X.2, Omega.norm, V, l.g.hat, y.1.g, y.2.g)
    
    #Method 2#
    theta.0.g <- solve(t(Cov.predict) %*% Cov.predict, t(Cov.predict)) %*% c(y.1.g, y.2.g)
    Est.2.g <- Estimator.2(var.pooled.g, C.1, X.1, X.2, Omega.norm, V, y.1.g, y.2.g, theta.0.g, 1e-6)
    beta.2.g <- Est.2.g$beta
    l.2.g <- Est.2.g$l
    Var.2 <- Est.2.g$var.theta
  }
